...
// 给实例容器派生一个"像个人电脑"的内部 hostname（替代 woc-wx- 这种容器/服务器特征）。
...
async function ensureImage(): Promise {
  try {
...
export async function runInstance(inst: Instance): Promise {
  ...
  const createOpts: Docker.ContainerCreateOptions = {
    ...
    Image: WECHAT_IMAGE,
    // 内部 hostname 伪装成"个人电脑"名（不再用 woc-wx-，那是容器/服务器特征）。
  ...
  try {
    ...
    // 它仍占着卷名 woc-data-，让后续删卷报 409。修复 #23 时发现 4 个此类残留。
    try {
...
export async function ensureRunning(inst: Instance): Promise {
  try {
...
export async function upgradeInstance(inst: Instance): Promise {
  try {
    await pullImage();
  } catch (e: any) {
...
export async function regenInstanceMachineId(inst: Instance): Promise {
  const hasHook = (
...
export async function stopInstance(inst: Instance): Promise {
  try {
...
export async function removeInstance(inst: Instance, purgeVolume: boolean): Promise {
  try {
    const c = docker.getContainer(inst.containerName);
...
export async function listOrphanVolumes(referencedVolumes: Set): Promise<
  Array<{ name: string; createdAt?: string; sizeBytes?: number }>
{
  ...
  const referenced = new Set([...referencedVolumes, ...containerRefs]);
...
export async function removeVolume(name: string): Promise {
...
export async function removeContainerById(idOrName: string): Promise {
...
export async function instanceMemoryMB(inst: Instance): Promise {
  try {
    const c = docker.getContainer(inst.containerName);
...
export async function instanceHttpHealthy(inst: Instance, timeoutMs = 8000): Promise {
...
export async function instanceRuntime(inst: Instance): Promise {
  try {
...
async function execCapture(inst: Instance, cmd: string[]): Promise {
  const c = docker.getContainer(inst.containerName);
...
export async function triggerWechat(inst: Instance, cmd: 'install' | 'update'): Promise {
  const c = docker.getContainer(inst.containerName);
...
export async function wechatStatus(inst: Instance): Promise {
  try {
...
export async function pullImage(onProgress?: (line: any) => void): Promise {
  return await new Promise((resolve, reject) => {
...
export async function buildDiagnostics(instances: Instance[], sinceMs: number, meta: Record): Promise {
  ...
  entries.push({
    ...
    content: [
      ...
      '  instances/.log 每个实例：容器状态 + 持久日志 + 实时容器日志',
      '',
...
export async function uploadToInstance(inst: Instance, name: string, content: Buffer): Promise {
...
export async function listInstanceFiles(inst: Instance): Promise {
...
export async function deleteInstanceFile(inst: Instance, name: string): Promise {
  if (!safeName(name)) throw new Error('文件名不合法');
  // argv 数组直传，不经 shell；safeName 已排除路径穿越
  await execCapture(inst, ['rm', '-f', \${TRANSFER_DIR}/${name}\]);
}

export async function downloadFromInstance(inst: Instance, name: string): Promise {
  if (!safeName(name)) throw new Error('文件名不合法');
  const c = docker.getContainer(inst.containerName);
...
export async function instanceLogs(inst: Instance, tail = 600): Promise {
  const c = docker.getContainer(inst.containerName);
...
export async function snapshotContainerLog(inst: Instance, reason: string): Promise {
  try {
...
export async function typeInInstance(inst: Instance, text: string): Promise {
...
export async function keyInInstance(inst: Instance, key: string): Promise {
...
export async function volMkdir(inst: Instance, rel: string): Promise {
  const abs = safeVolPath(rel);
  if (abs === VOL_ROOT) throw new Error('路径不合法');
  await execCapture(inst, ['mkdir', '-p', abs]);
}

export async function volMove(inst: Instance, fromRel: string, toRel: string): Promise {
...
export async function volDelete(inst: Instance, rel: string): Promise {
  const abs = safeVolPath(rel);
...
export async function volUploadFile(inst: Instance, rel: string, name: string, content: Buffer): Promise {
  if (!safeName(name)) throw new Error('文件名不合法');
...
export async function volExtractArchive(inst: Instance, rel: string, archive: Buffer): Promise {
  const dir = safeVolPath(rel);
  await execCapture(inst, ['mkdir', '-p', dir]);
  await docker.getContainer(inst.containerName).putArchive(maybeGunzip(archive), { path: dir });
}

export async function volDownloadFile(inst: Instance, rel: string): Promise {
  const abs = safeVolPath(rel);
...
export async function volBackupStream(inst: Instance): Promise {
...
export async function volRestoreArchive(inst: Instance, archive: Buffer): Promise {
